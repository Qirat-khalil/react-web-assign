...
//                         {imgs.map((img, index) => (
//                             <img key={index} src={img.image} alt="logo" ref={ani} className="w-20 md:w-30 object-contain" />
//                         ))}
//                     </div>
...
const SecondSec = () => {
    ...
    useEffect(() => {
        const logos = gsap.utils.toArray(containerRef.current.querySelectorAll("img"));
    ...
    return (
        <Container>
            <div className='flex flex-col justify-center items-center mt-8 py-8 mb-15 px-4'>
                ...
                <div 
                    ref={containerRef} 
                    className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 mt-12 mb-10 w-full place-items-center overflow-hidden'
                >
                    {imgs.map((img, index) => (
                        <img 
                            key={index} 
